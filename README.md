...
- [2026.09.12] 飞书 wiki3 §13(6) **CI + 灰度**过程闸：[feishu-ci-gray.md](docs/architecture/feishu-ci-gray.md) · `python3 scripts/check_ci_gray.py`。三个 required job 名不变（`structure` / `contracts` / `boundary`）；gray = required checks green + humans merge + no auto-merge；agents 停在 merge/production 之前。**不**编译 vendor。#41 / #42 may still be open — do not claim they merged。不改 XML / SCOREBOARD；跨机 UDP 仍 blocked；《3》–《6》仍 Hold。
