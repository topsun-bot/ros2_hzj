...
- [2026.09.12] 飞书 wiki3 §13(5) **一次一层**过程闸：[feishu-one-layer.md](docs/architecture/feishu-one-layer.md) · `python3 scripts/check_one_layer.py`。一个飞书 / 中间件层一个 PR；下一层另开 PR；不捆绑 Cega+memory+XML。#41 memory Hold may still be open — do not claim it merged。不改 XML / SCOREBOARD；跨机 UDP 仍 blocked；《3》–《6》仍 Hold。
