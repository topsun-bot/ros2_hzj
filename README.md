...
- [2026.09.12] 飞书《通信中间件》**memory** 下沉层 **Hold**：[feishu-memory-hold.md](docs/architecture/feishu-memory-hold.md) · `python3 scripts/check_memory_hold.py`。不落地 Loaned / Fast-DDS Data Sharing / Iceoryx 运行时 / Agnocast kmod。AUTO CMake ≠ zero-copy proven；`psmx_iox` 适配源码 ≠ vendor Iceoryx。Cega / Bridge 仍 Hold（#40）。不改 XML / SCOREBOARD。
