...
| [`2026-09-10-iter4/`](2026-09-10-iter4/README.md) | iter4：同一 send-buffer 池从 32 收成 16（`dynamic` 仍 true）。重测链 A 大包同拓扑；[`delta.md`](2026-09-10-iter4/delta.md) 只对照 iter3 同链同尺寸（主表）；相对 iter2-after 的 mid-size 诚实对照写在 delta 里。same-host 100/256 KiB 为主；1 MiB 须留在 iter3 噪声内。链 B 未重跑。 |
