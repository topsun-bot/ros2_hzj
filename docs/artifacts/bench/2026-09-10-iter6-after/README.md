...
**IMU packet + rate (same as Step A, nailed first):** **64 B / 200 Hz** (5 ms). Primary metric is **jitter** (RTT p95/p99 + inter-message interval). Do **not** claim success from p50/mean.
...
| [`chain_a_same_host/`](chain_a_same_host/summary.md) | A | `same-host` | ok — primary. BestEffort RTT p95 **+2.38%**, arrival \|I−5 ms\| p95 488→568; **not kept** |
| [`chain_a_cross_host_UDP/`](chain_a_cross_host_UDP/summary.md) | A | `cross-host-UDP` | blocked — 单机 VM |

链 B **未重跑**。落地 XML 回到 iter5 种子（中包 SHM 280000 / 2 MiB 保留；**1 MiB 赢面未擦**）。独占 / 过大 SHM 仍丢弃。《3》《4》《5》《6》仍 Hold。
