...
| **rmw** | 声明 [`vendor/rmw/rmw/include/rmw/rmw.h`](../../vendor/rmw/rmw/include/rmw/rmw.h)；装载 [`vendor/rmw_implementation/`](../../vendor/rmw_implementation/)；链 A [`vendor/rmw_fastrtps/`](../../vendor/rmw_fastrtps/)（`rmw_fastrtps_cpp`，域 **42**）；链 B [`vendor/rmw_cyclonedds/`](../../vendor/rmw_cyclonedds/)（`rmw_cyclonedds_cpp`，域 **0**）。身份闸 [`scripts/prove_rmw.py`](../../scripts/prove_rmw.py) | 自研 RMW、`rmw_zenoh`、把 rolling `rmw_*.h` 铺进 Humble、对照编译 vendor 当运行时 | **Allowed：** `RMW_IMPLEMENTATION` + `config/env` 切换已有实现；`prove_rmw.py` 证明加载了谁。**Hold：** 自定义 RMW、`rmw_zenoh`、Agnocast（Agnocast **不是** RMW） |
