...
| **memory** | 飞书把内存与 Executor 并列下沉。本仓只对照：Loaned / Fast-DDS Data Sharing / Iceoryx / Agnocast 见 [cn-jp-ros2-absorb.md](cn-jp-ros2-absorb.md)。Cyclone 快照已带 PSMX 适配源码 [`vendor/CycloneDDS/src/psmx_iox/`](../../vendor/CycloneDDS/src/psmx_iox/)（`ENABLE_ICEORYX` / `ENABLE_ICEORYX2` AUTO）；**Iceoryx 库本身**是 CMake 可选外部依赖，本仓**没有** `vendor/iceoryx` 树，默认不编进运行时（[VERSIONS.md](../../vendor/VERSIONS.md)） | 落地 Loaned、翻转现网 `data_sharing`、打开 Iceoryx 运行时 / Agnocast kmod、heaphook | **Hold：** Loaned / Data Sharing / 内存补丁 **不落地**。适配源码在 Cyclone 快照里 ≠ 已 vendor Iceoryx、≠ 已开零拷。Agnocast / zenoh 仍 Hold。零拷 **不**解锁跨机 UDP |
