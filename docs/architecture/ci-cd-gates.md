...
**Claude 代码审查（advisory，不是闸门）**：[`.github/workflows/claude-code-review.yml`](../../.github/workflows/claude-code-review.yml)。
`pull_request`（`opened` / `synchronize` / `reopened` / `ready_for_review`，draft 跳过）触发 `anthropics/claude-code-action@v1`，
按 [AGENTS.md](../../AGENTS.md) 的 Hold 清单与诚实标记审 diff，只用 `gh pr comment` / inline comment 留评论；**不**提交、**不**推送、**不**贴标签。
权限 `contents: read` + `pull-requests: write` + `issues: read` + `id-token: write`。需要仓库 secret `ANTHROPIC_API_KEY`。
**不要**把 `claude-review` 设为 required status check——合入闸门仍只有 `structure` / `contracts` / `boundary`（§5）。
