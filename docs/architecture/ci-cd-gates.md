...
- **输入由可信步骤准备**：action 之前的一个 `run` 步骤用固定参数把 diff（本地 `git diff <base sha> HEAD`，完整、无 compare API 的 300 文件上限，与 `boundary` job 的 base…HEAD 同形）和 **base 分支**的 [AGENTS.md](../../AGENTS.md)（`gh api …/contents/AGENTS.md?ref=<base sha>`）写到 `$RUNNER_TEMP/review/`。PR 若改 `AGENTS.md`，按 diff 当数据审，不当政策用。
- **工具边界靠 flag 强制，不靠 prompt**：`--allowedTools` 只有 `Read` / `Glob` / `Grep` / `LS` + 固定参数的 `update_claude_comment`；`--disallowedTools` 明确封 `Bash` / `Edit` / `Write` / `WebFetch` / `WebSearch` / file_ops / inline comment 等。模型查不了别的 PR / 别的仓，也没有 `gh pr comment --body-file` 这类外泄路径。
