...
| 权限 | `contents: read`、`pull-requests: write`、`issues: write`、`id-token: write`；工具白名单只含 `gh pr diff/view/comment` 与行内评论，不含写文件 / push；checkout 用 `persist-credentials: false`，模型可读的工作区里没有 `GITHUB_TOKEN` |
