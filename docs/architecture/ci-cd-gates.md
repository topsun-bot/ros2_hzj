...
另有一条**建议性**工作流 [`.github/workflows/claude-code-review.yml`](../../.github/workflows/claude-code-review.yml)：PR `opened` / `synchronize` / `ready_for_review` / `reopened`（仅**非 draft**、**同仓分支**；fork PR 在 `pull_request` 上拿不到 secret，显式跳过）时由 Claude Code（`anthropics/claude-code-action`，pin 到 commit SHA）读 diff，按 [AGENTS.md](../../AGENTS.md) 的 Hold 边界与本文闸门做代码审查，只发 PR 评论 / 行内评论，**不是** required check、**不**改代码、**不**替代 `boundary`。需要仓库 secret `ANTHROPIC_API_KEY`（或换成 `claude_code_oauth_token`）与安装 Claude GitHub App；secret 缺失时该 job 失败但不阻塞合并。
