...
- **人类批准 merge**。本仓不自动合入。
- 本仓灰度（wiki3 §13(6)，[feishu-ci-gray.md](feishu-ci-gray.md)）= required checks green + humans merge + no auto-merge。**不是** Mac HIL / Promptfoo / CVE / 90% LLM。
...
test -f docs/architecture/feishu-ci-gray.md
test -f AGENTS.md
...
test -f scripts/check_ci_gray.py
...
- [feishu-ci-gray.md](feishu-ci-gray.md) — wiki3 §13(6) CI + 灰度过程闸（required checks green + humans merge + no auto-merge；不编译 vendor；#41 / #42 may still be open）
