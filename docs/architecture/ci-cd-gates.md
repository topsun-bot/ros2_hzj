...
工作流：[`.github/workflows/claude-code-review.yml`](../../.github/workflows/claude-code-review.yml)（`anthropics/claude-code-action`，钉 v1.0.99 release commit SHA）。每个非 draft PR 在 `opened / synchronize / reopened / ready_for_review` 时自动跑一次 Claude 审查，按 [AGENTS.md](../../AGENTS.md) 的 Hold 规则找正确性 bug、Hold 越界、诚实标记（`STATUS: blocked` / `DoD: unmet`）与 eval 断言质量问题，结果以一条 sticky 总结评论（只匹配作者 `github-actions[bot]` 且以 `<!-- claude-code-review -->` 标记开头的评论，重跑原地更新，绝不改他人评论）+ 行内评论发回。

- **只评论、不改文件、不合入**；`structure` / `contracts` / `boundary` 仍是唯一必绿闸门，本 job **不要**设为 required status check。
- **信任边界 / 权限**：两个 action 都钉 commit SHA；action 硬编码按 tag 启动的 GitHub MCP server 镜像在前置 step 按 sha256 digest 预拉取并本地打 tag（`docker run` 用本地镜像，不再访问 registry）；用 job 自身的短期 `GITHUB_TOKEN`（`contents: read` / `pull-requests: write` / `issues: write`，无 `id-token`）。工作区根目录 checkout 的是**基线分支**（`pull_request.base.sha`，`persist-credentials: false`），所以 `AGENTS.md` 与 Claude 能 Read 的一切文件都来自基线；**PR head 不 checkout**（PR 塞进来的 `CLAUDE.md` / `.claude` 设置 / hooks 不会被自动加载），`use_commit_signing: true` 让 action 跳过 git 凭证配置，**token 不落 `.git/config`**。Claude **没有 shell**（Bash 禁用）、不能改文件、不能提交，只通过官方 GitHub MCP 的文本工具读 PR diff / 元数据并写评论；prompt 明确把 PR 标题 / 描述 / 评论 / diff 全部当作**不可信数据**。同仓 PR 作者本就有写权限，PR 改这个 workflow 本身与 `ci.yml` 一样是 GitHub 对任何带 secret 的 `pull_request` 工作流的标准信任边界；**不用 `pull_request_target`**（那会把 secret 交给 fork PR）。残余风险：进程环境里有 `ANTHROPIC_API_KEY`，读文件 + 写评论本身是文本外泄通道（任何工具型审查者都有）；缓解是最小、短期 token，fork PR 拿不到 secrets，该 secret 用独立限额的 workspace key。安全审计基线的变化见 [2026-09-vendor-cve-audit.md §2.2](../security/2026-09-vendor-cve-audit.md)。
