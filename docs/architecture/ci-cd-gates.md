...
- **一次性配置（repo admin）**：安装 [Claude GitHub App](https://github.com/apps/claude)；在仓库 secrets 加 `ANTHROPIC_API_KEY`（Claude API key）或 `CLAUDE_CODE_OAUTH_TOKEN`（`claude setup-token` 生成）。工作流同时传 `anthropic_api_key` 与 `claude_code_oauth_token` 两个 input，action 用非空的那个，不用改工作流。两个 secret 都缺失时，第一步 `Check review credentials` 打一条 `::notice` 然后 job **绿**退出（不装 plugin、不跑评审），不会因为尚未配置就出红叉；`ci.yml` 三个 job 不受影响。
