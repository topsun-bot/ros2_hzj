...
| 文件 | 作用 |
|---|---|
| `promptfooconfig.yaml` | 评估配置：1 个 custom provider + 18 个 seed 用例 |
...
- **seed 用例**（18 个）：13 个 gate 健康标记（其中 12 个额外断言实质 DDS/Hold/诚实性契约短语）、
  ...
  指纹回归用例（#17，见下节），以及 1 个 frozen-path guard 的负向自测用例（#18，见再下节）：
