...
| 41 | `evals/risk_matrix_guard_selftest.py` | `risk matrix guard selftest: PASS`、`15 negative, 2 non-flag, 2 healthy, 1 mutation`（删独立 range 中间项 `《4》`/`《5》` 但保留 `《3》–《6》` 区间必 `FAIL markers ... need 《4》/《5》`（端点《3》/《6》被区间字面满足、无法区分，有意不加）、删 R0 必需文件必 `FAIL missing`、ADR 去 `§9.4` 必 FAIL、矩阵删 `Agnocast`/`zenoh`/`Cega` 任一、或丢自身总 `Hold` 立场/跨机 `blocked` 诚实词、或丢 `Rolling ≠ Humble` 版本诚实对任一词必点名且其余 7 文件仍 ok（order 不连带）、R0/源映射/延迟方法/CI gates 文件保留但各丢唯一 marker Hold/vendor/wiki/structure 必 `FAIL markers` 点名；仅 `STATUS` 的 SCOREBOARD 与仅 `domainId>42` 的 fastdds.xml 最小锚点树保持绿；真实仓 + 纯复制树双健康；盲 `read_utf8` 总返回未篡改矩阵使 N1 漏报、恢复后重捕） |
| 42 | `evals/dual_chain_baseline_doc_selftest.py` | `dual-chain baseline doc guard selftest: PASS`、`31 negative, 3 non-flag, 2 healthy, 1 mutation`（删独立 paused 短语必 `FAIL paused`；注入英文 `p99`、中文紧邻 `链A的p99`、另一个分位 `p95` 均必 `FAIL percentiles`；ADR 去 `不重写 XML` 必仅点名 ADR `FAIL markers`；删 Unitree swap 文档必 `FAIL missing`；文件保留、各删核心安全 marker Hold/STATUS: blocked/只读 必仅点名 baseline 的 `FAIL markers`、不连带；swap 文档保留、各删内容 marker drop-in FAIL/0.10.2/11.0.1 必仅点名 swap 的 `FAIL markers`、baseline 不连带；R0 文档保留删 Hold、源映射保留删 vendor/不是复现 必仅点名该文件的 `FAIL markers`、不连带；ADR 保留删 FastDDS + Cyclone/SCOREBOARD/baseline 回指 必仅点名 ADR 的 `FAIL markers`、不连带；baseline 保留删 Not Feishu field proof/派生自 必仅点名 baseline 的 `FAIL markers`、不连带；baseline 保留删双链 RMW/domain 身份词 必仅点名 baseline 的 `FAIL markers`、不连带 chain A/B；baseline 保留删 cross-host/three-chain blocked 诚实词或 Chain B unset-URI 契约词 必仅点名 baseline 的 `FAIL markers`、不连带 chain；baseline 保留、各删《3》/《4》/《5》/《6》Hold 范围标记 必仅点名 baseline 的 `FAIL markers`、不连带；空 SCOREBOARD、domainId 99 的任意 XML、仅含政策词“分位数”无裸 pNN 的句子均 exit0；真实树+纯复制树双健康；盲百分位正则漏报 p99、恢复重捕） |
...
- `risk_matrix_guard_selftest.py`（中缀 `risk_matrix_guard`；纯标准库，把 guard 实读的 8 个文件——6 份内容文档 + 只读的 fastdds.xml / SCOREBOARD.md——**复制进 tempdir** 后逐树变异，经可注入的 `render(root=...)` 驱动，绝不写仓库、绝不改两个冻结文件）钉：
  - **15 negative**：N1 删独立 range 中间项 `《4》`、**N11 删另一中间项 `《5》`**（只删表格 `《5》Promptfoo`、区间完整保留）→ exit1 且逐字 `FAIL markers ... (need 《4》/《5》)`；探针证实端点 `《3》`/`《6》` 被区间字面《3》–《6》满足、删表格 standalone 仍 exit0（substring 无法区分 standalone 与区间，属物理不可触发分支，收紧需改 guard 检测逻辑、非 eval-only，有意不造负向）；N2 删 R0 必需文件 → exit1 `FAIL missing`；N3 ADR 去 `§9.4` → exit1（证明 marker 逐文件强制，不只查矩阵）；N4/N5/N6 从矩阵各删一个核心 Hold 禁令词 `Agnocast`/`zenoh`/`Cega` → exit1 逐字 `FAIL markers ... (need <词>)`，且其余 7 个文件仍打印 `ok file`（不连带）；三词各占一个 case，与《3》–《6》逐 token 同构，防止只从 marker 元组删掉其一时被另两个掩盖；N12/N13 矩阵文件**保留**、删自身总 `Hold` 立场或跨机 `blocked` 诚实词（跨机 UDP 本机必须标 blocked）→ exit1 逐字 `FAIL markers ... (need Hold/blocked)`，其余 7 文件仍 `ok file`、order 检查不连带（N4–N6 只覆盖具体禁令词、N7 的 Hold 在 R0 文件，matrix 自身丢总 Hold/blocked 此前无断言）；N14/N15 矩阵文件**保留**、删版本诚实对 `Humble`/`Rolling` 任一词（matrix 必须保留 Rolling ≠ Humble 判决——vendor 是 rolling/master 快照、运行时目标 Humble，严禁覆盖发行版树）→ exit1 逐字 `FAIL markers ... (need Humble/Rolling)`，其余 7 文件仍 `ok file`、order 不连带（matrix 保留却删版本诚实对任一词此前无断言）；N7/N8/N9/N10 文件**保留**、各删唯一 marker——R0 丢 `Hold`、源映射丢 `vendor`、延迟方法丢 `wiki`、CI gates 丢 `structure` → exit1 逐字 `FAIL markers ... (need <词>)`，其余 7 文件仍 `ok file`（N2 只覆盖文件缺失 FAIL missing，present-but-weakened 此前无断言，源映射/方法/gates 三文件原本零负向）；
