...
| 41 | `evals/risk_matrix_guard_selftest.py` | `risk matrix guard selftest: PASS`、`6 negative, 2 non-flag, 2 healthy, 1 mutation`（删独立 `《4》` 但保留 `《3》–《6》` 区间必 `FAIL markers ... need 《4》`、删 R0 必需文件必 `FAIL missing`、ADR 去 `§9.4` 必 FAIL、矩阵删 `Agnocast`/`zenoh`/`Cega` 任一必 `FAIL markers` 点名且其余 7 文件仍 ok；仅 `STATUS` 的 SCOREBOARD 与仅 `domainId>42` 的 fastdds.xml 最小锚点树保持绿；真实仓 + 纯复制树双健康；盲 `read_utf8` 总返回未篡改矩阵使 N1 漏报、恢复后重捕） |
...
- `risk_matrix_guard_selftest.py`（中缀 `risk_matrix_guard`；纯标准库，把 guard 实读的 8 个文件——6 份内容文档 + 只读的 fastdds.xml / SCOREBOARD.md——**复制进 tempdir** 后逐树变异，经可注入的 `render(root=...)` 驱动，绝不写仓库、绝不改两个冻结文件）钉：
  - **6 negative**：N1 删独立 `《4》`、保留区间 → exit1 且逐字 `FAIL markers ... (need 《4》)`；N2 删 R0 必需文件 → exit1 `FAIL missing`；N3 ADR 去 `§9.4` → exit1（证明 marker 逐文件强制，不只查矩阵）；N4/N5/N6 从矩阵各删一个核心 Hold 禁令词 `Agnocast`/`zenoh`/`Cega` → exit1 逐字 `FAIL markers ... (need <词>)`，且其余 7 个文件仍打印 `ok file`（不连带）；三词各占一个 case，与《3》–《6》逐 token 同构，防止只从 marker 元组删掉其一时被另两个掩盖；
