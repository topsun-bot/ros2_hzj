...
| 42 | `evals/dual_chain_baseline_doc_selftest.py` | `dual-chain baseline doc guard selftest: PASS`、`18 negative, 3 non-flag, 2 healthy, 1 mutation`（删独立 paused 短语必 `FAIL paused`；注入英文 `p99`、中文紧邻 `链A的p99`、另一个分位 `p95` 均必 `FAIL percentiles`；ADR 去 `不重写 XML` 必仅点名 ADR `FAIL markers`；删 Unitree swap 文档必 `FAIL missing`；文件保留、各删核心安全 marker Hold/STATUS: blocked/只读 必仅点名 baseline 的 `FAIL markers`、不连带；swap 文档保留、各删内容 marker drop-in FAIL/0.10.2/11.0.1 必仅点名 swap 的 `FAIL markers`、baseline 不连带；R0 文档保留删 Hold、源映射保留删 vendor/不是复现 必仅点名该文件的 `FAIL markers`、不连带；ADR 保留删 FastDDS + Cyclone/SCOREBOARD/baseline 回指 必仅点名 ADR 的 `FAIL markers`、不连带；空 SCOREBOARD、domainId 99 的任意 XML、仅含政策词“分位数”无裸 pNN 的句子均 exit0；真实树+纯复制树双健康；盲百分位正则漏报 p99、恢复重捕） |
