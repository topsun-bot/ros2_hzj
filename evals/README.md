...
| 41 | `evals/risk_matrix_guard_selftest.py` | `risk matrix guard selftest: PASS`、`13 negative, 2 non-flag, 2 healthy, 1 mutation`（删独立 range 中间项 `《4》`/`《5》` 但保留 `《3》–《6》` 区间必 `FAIL markers ... need 《4》/《5》`（端点《3》/《6》被区间字面满足、无法区分，有意不加）、删 R0 必需文件必 `FAIL missing`、ADR 去 `§9.4` 必 FAIL、矩阵删 `Agnocast`/`zenoh`/`Cega` 任一、或丢自身总 `Hold` 立场/跨机 `blocked` 诚实词必点名且其余 7 文件仍 ok（order 不连带）、R0/源映射/延迟方法/CI gates 文件保留但各丢唯一 marker Hold/vendor/wiki/structure 必 `FAIL markers` 点名；仅 `STATUS` 的 SCOREBOARD 与仅 `domainId>42` 的 fastdds.xml 最小锚点树保持绿；真实仓 + 纯复制树双健康；盲 `read_utf8` 总返回未篡改矩阵使 N1 漏报、恢复后重捕） |
