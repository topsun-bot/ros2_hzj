...
iter7 只改同一条 `shm_midsize` 的 `healthy_check_timeout_ms`（1000 → 10000）。Humble Fast-DDS 2.6 默认 1000 ms；SHM 端口监视线程超时后会拿 `empty_cv_mutex`。IMU 64 B / 200 Hz 单飞约 2 s，预测 10 s 把这次检查踢出 p95/p99 窗口。不是 `port_queue_capacity`（已丢）。中包 SHM 280000 / 2 MiB、socket 2 MiB、`send_buffers` 32 / `dynamic=false` 不动。1 MiB 仍走 builtin 分片（样本 > 280000）。独占 / 过大 SHM 仍丢弃。主指标仍是 jitter，不是 p50。假设写在 `docs/artifacts/bench/2026-09-11-iter7/change.md`。不是现网证明。
