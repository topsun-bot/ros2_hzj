...
iter6 探测过把同一条 `shm_midsize` 的 `port_queue_capacity` 从 512 收到 64。IMU Step A same-host BestEffort 64 B / 200 Hz p50 965 → 999 µs（+3.5%，噪声 / 略差），max 变差；Reliable 持平。**未保留**。落地 XML 仍是 iter5 种子（280000 / 2 MiB，builtin 开，socket + send_buffers 不动）。独占 / 过大 SHM 仍丢弃。记录在 `docs/artifacts/bench/2026-09-10-iter6-after/change.md`。不是现网证明。
